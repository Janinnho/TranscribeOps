...
```bash
# Full stack via podman-compose (production config)
sudo podman-compose -f podman-compose.yml up --build

# Web-app only via docker-compose
cd web-app && docker compose up --build

# Rebuild only web and worker (preserving redis/whisper)
sudo podman-compose -f podman-compose.yml build web worker
```

### Deployment Notes

- Production uses `podman-compose.yml` at repo root (pod with web, worker, redis, whisper)
- `web-app/docker-compose.yml` is for standalone web-app development
- Named volumes: `transcribeops-db` (SQLite), `transcribeops-uploads`, `transcribeops-redis`, `transcribeops-whisper-cache`
- Audio storage bind-mounted from `/mnt/storagebox/TranscribeOps-Audio`
