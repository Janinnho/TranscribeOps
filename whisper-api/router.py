...
# Response headers worth forwarding from instance workers to the client.
# Everything else is either hop-by-hop (Connection, Transfer-Encoding, …) or
# regenerated by Flask (Content-Length).
_FORWARD_HEADERS = ("Content-Type", "Retry-After", "Content-Disposition", "Cache-Control")
...
class InstanceRouter:
    ...
    def resolve(self, model_param: str | None) -> tuple[str, dict | None]:
        ...
        if not name or name.lower() == "whisper-1":
            return ("local", None)
        # Case-insensitive throughout — instance names are unique
        # case-insensitively, so resolution must match that rule.
        folded = name.lower()
        rows = self._instances()
        for row in rows:
            if row["name"].lower() == folded:
                return ("proxy", row)
        try:
            if any(folded == a.lower() for a in self._local_aliases_fn()):
                return ("local", None)
        except Exception:
            logger.exception("local_aliases_fn failed")
        by_model = [r for r in rows if r["model"].lower() == folded]
        if len(by_model) == 1:
