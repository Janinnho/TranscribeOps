...
TRANSLATIONS: dict[str, dict[str, str]] = {
    "de": {
        ...
        "Each instance loads its own model and listens on its own port. Multiple instances run in parallel — ideal for express mode without a separate container.": "Jede Instanz lädt ein eigenes Modell und lauscht auf einem eigenen Port. Mehrere Instanzen laufen parallel — ideal für Express-Modus ohne separaten Container.",
        "Port range: %(range)s": "Port-Range: %(range)s",
        "New instance": "Neue Instanz",
        "e.g. express": "z.B. express",
        "Port (empty = automatic)": "Port (leer = automatisch)",
