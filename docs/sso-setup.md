...
7. Click **SSO-Einstellungen speichern**
...
|-------|-------------|---------|
| **E-Mail Header** | HTTP header containing the authenticated user's email | `Cf-Access-Authenticated-User-Email` |
...
2. **Configure TranscribeOps SSO settings**
   - SSO Methode: **Header-basiert**
   - E-Mail Header: `Cf-Access-Authenticated-User-Email`
...
2. **Configure TranscribeOps SSO settings**
   - SSO Methode: **Header-basiert**
   - E-Mail Header: `X-authentik-email`
...
| **E-Mail Claim** | JWT claim containing the email address | `email` |
...
5. **Configure TranscribeOps SSO settings**
   - SSO Methode: **OpenID Connect (OIDC)**
...
### Header-based SSO: "Benutzer nicht gefunden und automatische Erstellung ist deaktiviert"
...
- Enable "Benutzer automatisch erstellen" in SSO settings
...
### OIDC: "OIDC ist nicht vollständig konfiguriert"
...
### OIDC: "Keine E-Mail-Adresse im OIDC-Token gefunden"

The identity provider didn't include an email in the token. Check:
- The `email` scope is included in the Scopes setting
- The E-Mail Claim setting matches the actual claim name in the provider's tokens
...
Use the manual login at `/manuell-login` with your local admin credentials to access the admin portal and adjust SSO settings.
