...
## Whisper API Service starten

Der Whisper-API-Service stellt eine OpenAI-kompatible Transkriptions-API bereit, die das `faster-whisper`-Modell lokal ausführt.
...
Die Konfiguration erfolgt über Umgebungsvariablen in `whisper-api/docker-compose.yml`:
...
**Zwingend für Produktion:** Setze einen sicheren, zufälligen `SECRET_KEY`:
...
Standardmäßig werden Audio-Dateien unter `~/Downloads/TranscribeOps-Audio` gemountet. Passe den Pfad in `docker-compose.yml` an:
...
3. **docker-compose.yml** — Ihre Konfiguration
...
# Whisper API aktualisieren (falls nötig)
cd ../whisper-api
docker compose up -d --build
```
...
docker volume rm web-app_db_data web-app_upload_data web-app_redis_data
docker volume rm whisper-api_whisper_cache
