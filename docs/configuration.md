...
Die gesamte Konfiguration erfolgt über eine einzige Compose-Datei im Root-Verzeichnis. Secrets werden aus der `.env`-Datei gelesen.

```bash
# Setup
cp docker-compose.example.yml docker-compose.yml
cp .env.example .env
# .env anpassen (SECRET_KEY, HF_TOKEN, etc.)
```

> **Wichtig:** `web` und `worker` müssen **identische** Volumes und Umgebungsvariablen haben, da beide auf die gleiche Datenbank und die gleichen Dateien zugreifen. Siehe `docker-compose.example.yml` für die vollständige Referenz.
